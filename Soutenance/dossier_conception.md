...
Je me suis demandée comment créer une expérience d’échange simple et intuitive, comment réduire les frictions, comment donner envie de s’inscrire, comment faciliter la navigation sur mobile, et comment faire en sorte que les utilisateurs se sentent en confiance sur une plateforme entièrement nouvelle.
...
Chaque étape a été pensée pour être fluidet accessible.
...
TalentÉkô est un projet complet.
...
J’ai imaginé des personas représentatifs, comme :
**Julie** : jeune femme créative qui souhaite transmettre son savoir en dessin tout en trouvant quelqu’un pour l’aider en bricolage.
**Marc** : autodidacte passionné d’informatique, qui propose ses réparations et cherche à progresser en cuisine.
**Anna** : développeuse organisée, qui souhaite offrir une aide administrative et, en échange, apprendre la guitare.
...
J’ai conçu les maquettes avec Figma, en adoptant une approche destktop first au vue de notre apprentissage neanmois je sais que la bonne pratique et le mobile first. C'est d'ailleur plus simple dans ce sens!
...
L’inscription est intuitive et totalement fonctionnelle, permettant à n’importe quel utilisateur de rejoindre TalentÉkô en quelques minutes.
...
# IX — Déploiement
Le déploiement de **TalentÉkô** a été réalisé sur un hébergement mutualisé **o2switch**, en utilisant une approche moderne et professionnelle : déploiement continu via GitHub, environnement sécurisé, configuration Symfony optimisée pour la production, gestion des permissions et préparation du cache d’images.

Cette partie détaille l’ensemble des étapes permettant de mettre en ligne une application Symfony fonctionnelle, performante et stable.


## **9.1 Préparation du dépôt GitHub**

L’intégralité du projet TalentÉkô est versionnée sur GitHub afin de garantir :
-   le suivi des modifications,
-   la collaboration potentielle,
-   la sécurité et la sauvegarde du code,
-   un déploiement propre depuis un dépôt centralisé.


### Commandes utilisées

git init
git remote add origin https://github.com/toncompte/talenteko.git
git add .
git commit -m "Initial commit - TalentEko" git push -u origin main`


## **9.2 Connexion au serveur o2switch**

o2switch met à disposition un accès SSH permettant d’exécuter toutes les commandes nécessaires au déploiement.

### Commande de connexion (réelle) :

`ssh ch******@reveil.o2switch.net`

## **9.3 Récupération du projet depuis GitHub**

Arrivé sur le serveur :
`cd ~/public_html/
git clone https://github.com/toncompte/talenteko.git`

Puis :
`cd talenteko`

## **9.4 Installation des dépendances**

### **PHP (Composer)**

`composer install --no-dev --optimize-autoloader`

### **Node/Tailwind/Encore (local, avant envoi)**

o2switch ne permet **pas** l’installation de Node.js en mutualisé.
Il faut donc compiler les assets **en local**, puis envoyer `public/build`.


## **9.5 Configuration de l’environnement**

Création du fichier `.env.local` **sur le serveur** :

`nano .env.local`

Contenu adapté à o2switch :

`APP_ENV=prod APP_DEBUG=0  APP_SECRET=xxxxxxxxxxxxxxxxxxxxxxxx DATABASE_URL="mysql://USER:PASS@127.0.0.1:3306/DBNAME?charset=utf8mb4&serverVersion=mariadb-10.5.0"  MAILER_DSN=null://null APP_MAILER_FROM=no-reply@talenteko.fr`

----------

## **9.6 Réglage des permissions**

Symfony a besoin que certains dossiers soient accessibles en écriture :
`chmod -R 775 var chmod -R 775 public chmod 666 var/log/prod.log`

## **9.7 Migration de la base de données**

`php bin/console doctrine:migrations:migrate --no-interaction`

## **9.8 Configuration Apache (.htaccess)**

Fichier déjà présent dans `public/.htaccess`.
Il redirige correctement vers `public/index.php`.

## **9.9 Préparation du cache Symfony (vrai déploiement)**
...
## **9.10 Préparation du cache des images (LiipImagine)**

`php bin/console liip:imagine:cache:remove
php bin/console liip:imagine:cache:resolve uploads/listings --filter=listing_card
php bin/console liip:imagine:cache:resolve uploads/listings --filter=listing_show
php bin/console liip:imagine:cache:resolve uploads/listings --filter=avatar_thumb`

----------

## **9.11 Vérification finale**

-   Le site se charge sans erreur en prod
-   Les images s’affichent
-   Les routes fonctionnent
-   La navigation est fluide
-   Le mode production est actif
-   Le cache est bien généré

TalentÉkô est alors **officiellement déployé** et accessible par URL publique.
...
En développant TalentÉkô, j’ai compris que coder, ce n’est pas seulement écrire des lignes de PHP ou intégrer des composants Tailwind ; c’est imaginer, structurer, concevoir, tester, corriger, optimiser!
...
**Annexe 3 —  Captures d’écran du site final**
