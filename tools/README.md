...
Ce dossier contient divers scripts utilisés pour garantir la **qualité du code** et la **cohérence du projet** TalentÉkô.
Ils ne sont pas nécessaires à l’exécution de l’application, mais permettent de nettoyer, corriger ou organiser le projet.
...
1.  Ainsi, on peut revenir en arrière si nécessaire.
2.  Ne jamais lancer les scripts sur `vendor/` ou `node_modules/`.
3.  Les scripts sont configurés pour ignorer ces dossiers.
4.  Utiliser `git diff` après chaque exécution
5.  pour valider les changements avant commit.
